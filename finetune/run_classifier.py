...
def count_labels_num(path):
    ...
    if 'lcqmc' in path or 'Chn' in path:
        return 2
    if 'xnli' in path:
        return 3
    with open(path, mode="r", encoding="utf-8") as f:
        for line in f:
            di = json.loads(line.rstrip())
            #print(di)
            labels_set.add(di['label'])
...
def read_dataset(args, path):
    ...
    with open(path, mode="r", encoding="utf-8") as f:
        for line_id, line in enumerate(f):
            if 'xnli' in path or 'lcqmc' in path:
                example = {}
                example['sentence1'], example['sentence2'], tgt = line.strip().split("\t")
                if tgt == 'entailment':
                    tgt = 0
                elif tgt == 'contradiction' or tgt == 'contradictory':
                    tgt = 1
                elif tgt == 'neutral':
                    tgt = 2
                tgt = int(tgt)
                ...
                example = {}
                if line.startswith('label') or line.startswith('qid'):
                    continue
                if 'train' in path:
                    tgt, example['sentence'] = line.strip().split("\t")
                elif 'dev' in path:
                    _, tgt, example['sentence'] = line.strip().split("\t")
                #print(tgt)
                tgt = int(tgt)
                #print(example, tgt)
            else:
                example = json.loads(line)
                if "afqmc" in path or "ifly" in path or "csl" in path:
                    tgt = int(example['label'])
                elif "tnews" in path:
                    tgt = int(example['label']) - 100
                    if tgt > 5 and tgt < 11:
                        tgt -= 1
                    elif tgt > 11:
                        tgt -= 2
                elif "cmnli" in path or "ocnli" in path:
                    if example['label'] == '-':
                        continue
                    ...
                    tgt = 0 if example['label'] == "true" else 1
    ...
    for epoch in range(1, args.epochs_num + 1):
        ...
        for i, (src_batch, tgt_batch, seg_batch, soft_tgt_batch) in enumerate(batch_loader(batch_size, src, tgt, seg, soft_tgt)):
            ...
            if global_steps  % args.report_steps == 0:
                ...
                print("Epoch id: {}, Training steps: {}, best accs: {:.3f}".format(epoch, global_steps, best_result))
